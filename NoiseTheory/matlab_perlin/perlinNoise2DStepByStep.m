% NAIVE Perlin Nose 2D, step by step
...
function noise = perlinNoise2DStepByStep(num_points, frequency, amplitude)
  ...
  % We will keep our gradient vector by components in 2 separated matrices, but
  % this is an arbitrary choice
  gradient_vec_x = zeros(num_lattice, num_lattice);
  gradient_vec_y = zeros(num_lattice, num_lattice);

  for i=1:num_lattice
    for j=1:num_lattice
        vec = getRandomGradient();
        gradient_vec_x(i,j) = vec(1);
        gradient_vec_y(i,j) = vec(2);
    endfor
  endfor
  ...
  for i=1:num_points
    for j=1:num_points
      ...
      grad00 = [gradient_vec_x(x0,y0) gradient_vec_y(x0,y0)];
      grad01 = [gradient_vec_x(x0,y1) gradient_vec_y(x0,y1)];
      grad10 = [gradient_vec_x(x1,y0) gradient_vec_y(x1,y0)];
      grad11 = [gradient_vec_x(x1,y1) gradient_vec_y(x1,y1)];
...
function res = getRandomGradient()
    rand_coords = (rand(2,1) * 2) -1;  % yields 2 random values ranging from -1 to 1
    res = rand_coords / norm(rand_coords);
    res = rand_coords;
end;
